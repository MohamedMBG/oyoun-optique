...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-xl border p-6 pr-8 shadow-lg transition-all",
  {
    variants: {
      variant: {
        default: "border-border bg-charcoal-card text-foreground",
        destructive:
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-lg border border-border bg-transparent px-3 text-sm font-medium transition-colors",
      "hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-gold disabled:pointer-events-none disabled:opacity-50",
      className
    )}
    {...props}
  />
